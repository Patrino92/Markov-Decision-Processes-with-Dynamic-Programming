...
class Dynamic_Programming:
    ...
    def value_iteration(self, env, gamma=1.0, theta=0.001):
        ...
        while True:
            ...
            for state in env.states:
                ...
                for action in env.actions:
                    ...
                    max_val = max(max_val, reward + gamma * V_s[next_state])
                V_s[state] = max_val
        ...
        #     for action in env.actions:
        #         next_state, reward = env.transition_function(state, action)
        #         value = reward + gamma * V_s[next_state]
        #         if value > max_val:
        #             max_val = value
        #             max_action = action
        ...
        while True:
            ...
            for state in env.states:
                for action_idx, action in enumerate(env.actions):
                    curr_state_val = Q_sa[state, action_idx]
                    next_state, reward = env.transition_function(state, action)
                    max_val = np.max(Q_sa[next_state, :])
                    Q_sa[state, action_idx] = reward + gamma * max_val
                    delta = max(delta, abs(curr_state_val - Q_sa[state, action_idx]))
            if delta < theta:
                break
            print(delta)
        self.Q_sa = Q_sa
    ...
    def execute_policy(self, env, table='V'):
        ...
        while not env.terminal:
            ...
            if table == 'V' and self.V_s is not None:
                ...
                greedy_action = env.actions[np.argmax(self.Q_sa[current_state])]
            ...
            while True:
                if greedy_action is not None:
                    ...
                    your_choice = input(
                        'Choose an action by typing it in full, then hit enter. Available are {}'.format(env.actions))
                ...
                if your_choice == "" and greedy_action is not None:
                    ...
                    try:
                        ...
                        print('{} is not a valid action. Available actions are {}. Try again'.format(your_choice,
                                                                                                     env.actions))
